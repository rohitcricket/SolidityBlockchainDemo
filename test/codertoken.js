...
contract("CoderToken", function(accounts) {
	it("Should put 15000 CoderToken in the first account", function() {
		return CoderToken.deployed()
			.then(function(instance) {
				return instance.balanceOf.call(accounts[0]);
			})
			.then(function(balance) {
				assert.equal(balance.valueOf(), 15000, "15,000 CoderToken wasn't in the first account");
			});
	});
	it("Should send 10 CoderToken correctly", function() {
		var token;
		...
		return CoderToken.deployed()
			...
			.then(function(balance) {
				...
				assert.equal(
					...
					"Amount was not successfully transferred from sender."
				);
				assert.equal(
					account_two_ending_balance,
					account_two_starting_balance + amount,
					"Amount was not successfully sent to receiver."
				);
			});
	});
});
